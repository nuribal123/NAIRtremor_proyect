...
class SeqLSTMModel(nn.Module):
    def __init__(self, input_dim=1, hidden_dim=50, output_dim=2, num_layers=2, dropout_prob=0.153248867):
...
def evaluate_and_plot(loader, title="Evaluación"):
    ...
    def plot_example(i):
        ...
        plt.plot(t, y_real[:, 0], label='ECRL real', color='green')
        plt.plot(t, y_pred_plot[:, 0], '--', label='ECRL pred', color='lightgreen')
        plt.plot(t, y_real[:, 1], label='FCR real', color='orange')
...
plt.plot(futuro_ms, pred.cpu().numpy()[0, :, 1], label='FCR predicho')
